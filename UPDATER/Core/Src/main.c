...
uint8_t pKeyAES[16] = { 0x12,0x34,0x56,0x78,0x91,0x01,0x11,0x21,0x31,0x41,
						0x51,0x61,0x71,0x81,0x92,0x0A };

uint8_t nonce_counter[12] = { 0x34,0x48,0x29,0x35,0x46,0x83,0x65,0xf7,0xa4,0xb4,0x8d,0x02 };

uint8_t HeaderAES[16] = { 0x42,0x06,0x92,0x07,0x70,0xff,0x06,0xb5,0x65,0x73,
...
// huart is used as wrapper for generated UART_HandleTypeDef handle
UART_HandleTypeDef huart;

// holder for updating method selection
uint8_t FullOrPatch = UPDATER_STATE_UPDATE_PATCH;
unsigned char calculated_tag[16] = { 0 };

// initialize ring buffers for reception and transmission
...
//needed for send to host function
...
int up_main(void)
{
	...
	//copy everything from specific USARTx handler
	memcpy(&huart, &huart2, sizeof(huart));
	__HAL_UART_ENABLE_IT(&huart, UART_IT_ERR);
	__HAL_UART_ENABLE_IT(&huart, UART_IT_RXNE);

	// print available updater options to the terminal
...
// TODO: TEST
		...
		else
		{
			...
			USART_SendString((uint8_t*)"Received data couldn't be authentificated\n\r");
			HAL_Delay(1000);
		...
		if (crc_received == crc_calculated)
		{
			if (0x1 == a_ReadRxData[1])
			{
				...
				else if (*pUploadAddress == 0xFF)
				{
					...
					{
						X_SendCan();
						USART_SendString((uint8_t*)"Size of the firmware is too big. Aborting!\n\r");
						HAL_Delay(1000);
...
void DownloadUserApplication(void)
{
	...
	while (1)
	{
		if (SET == RingBuffer_Read(&rx_byte, &rxBuff))
		{
			if (X_ACK == rx_byte)
			{
				USART_SendString((uint8_t*)"Transfer complete successfully.\n\r");
				UpdaterOptionSelected = RESET;
			...
			{
				USART_SendString((uint8_t*)"Failed to transmit. No ACK after transferring.\n\r");
				UpdaterOptionSelected = RESET;
				first_packet_complete = RESET;
				ShowMessage(MSG_SELECT_OPTION);
				rx_byte = 0;
...
void ShowMessage(MESSAGE_t msg)
{
	switch (msg)
	{
	case MSG_JUMP_TO_LOADER:
	{
		USART_SendString((uint8_t*)" Jumping to application loader...\n\r");
		break;
	}
	case MSG_SELECT_OPTION:
	{
		USART_SendString((uint8_t*)"Please select an option:\n\r\n\r");
		USART_SendString((uint8_t*)" > 'SHIFT+U' --> Hold 'SHIFT' and then press 'U' key to upload new firmware\n\r\n\r");
		USART_SendString((uint8_t*)" > 'SHIFT+D' --> Hold 'SHIFT' and then press 'D' key to download current firmware to host\n\r\n\r");
		USART_SendString((uint8_t*)" > 'SHIFT+V' --> Hold 'SHIFT' and then press 'V' key to print version of the current user application\n\r\n\r");
		USART_SendString((uint8_t*)" > 'SHIFT+E' --> Hold 'SHIFT' and then press 'E' key to erase current user application\n\r\n\r");
		USART_SendString((uint8_t*)" > 'SHIFT+B' --> Hold 'SHIFT' and then press 'B' key to load backup firmware into user application space\n\r\n\r");
		USART_SendString((uint8_t*)" > 'ENTER'   --> Press 'ENTER' for regular boot\n\r");
		break;
	}
	case MSG_SLCT_UPDT_METHOD:
	{
		USART_SendString((uint8_t*)"Please select an update method:\n\r\n\r");
		USART_SendString((uint8_t*)" > 'SHIFT+A' --> Hold 'SHIFT' and then press 'A' key to install major update(current version will be removed irretrievably)\n\r\n\r");
		USART_SendString((uint8_t*)" > 'SHIFT+P' --> Hold 'SHIFT' and then press 'P' key to apply a patch to currently installed application\n\r\n\r");
		break;
	}
	...
	{
		USART_SendString((uint8_t*)"Firmware update in progress! Please wait until the operation is over!\n\r");
		break;
	}
	case MSG_ASK_FOR_A_FILE:
	{
		USART_SendString((uint8_t*)"Send a new binary file using XMODEM protocol to update device firmware.\n\r");
		break;
	}
	case MSG_CRC_ERR:
	{

		USART_SendString((uint8_t*)"CRC error!!!\n\r\n\r");
		break;
	}
	case MSG_FLASH_ERR_WRITE:
	{
		USART_SendString((uint8_t*)"Data length doesn't fit any block size format!\n\r\n\r");
		break;
	}
	case MSG_INDEX1_ERR:
	{
...
void Read_Key(void)
{
	while (0 == rx_byte)
	{
		...
		case 'U':
		{
			...
			ShowMessage(MSG_SLCT_UPDT_METHOD);
		...
		case 'D':
		{
			ShowMessage(MSG_FILE_RDY_2_SEND);
		...
		{// jump to app loader
			...
			ShowMessage(MSG_JUMP_TO_LOADER);
			clear_screen();
			set_cursor_position(0, 0);
			JumpToAppLoader();
			break;
		...
		{//erase user application that is in slot 2
			...
			{
				USART_SendString((uint8_t*)"Flash memory is already empty!\n\r");
			}
			else
			{
				USART_SendString((uint8_t*)"Erasing flash. Please wait...\n\r");
				FLASH_EraseSector(SLOT_2_APP_ADDR - 0x20000);
				FLASH_EraseSector(SLOT_2_APP_ADDR);
				USART_SendString((uint8_t*)"Done.\n\r");
			}
			rx_byte = 0;
			break;
		...
		case 'B':
		{
			...
			{
				USART_SendString((uint8_t*)"No backup data available!\n\r");
				rx_byte = 0;
				break;
			}
			if (0xFFFFFFFF == *((uint32_t*)SLOT_2_VER_ADDR))
			{
				...
				USART_SendString((uint8_t*)"Copying from backup. Please wait...\n\r");
				HAL_GPIO_TogglePin(GREEN_GPIO_Port, GREEN_Pin);
			...
			{
				USART_SendString((uint8_t*)"You have to erase user application first!\n\r");
			}
			rx_byte = 0;
			break;
		}
