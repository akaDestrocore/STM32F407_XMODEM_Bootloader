...
void USART2_IRQHandler(void)
{
  /* USER CODE BEGIN USART2_IRQn 0 */
	if((READ_BIT(huart.Instance->SR, UART_FLAG_RXNE)) && (READ_BIT(huart.Instance->CR1,USART_CR1_RXNEIE)))
	{
		//store data from RX FIFO to RX ring buffer so data is not lost
		 uint8_t receivedData = huart.Instance->DR;
		 RingBuffer_Write(&rxBuff, receivedData);
	}
	//if TX is empty and TX interrupt is enabled
	if((READ_BIT(huart.Instance->SR, UART_FLAG_TXE)) && (READ_BIT(huart.Instance->CR1, USART_CR1_TXEIE)))
	{
		TxInProgress = RESET;
